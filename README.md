...
`The actor_id in each table are linked by only one actor_id from the actor table, but there can be many instances of that actor_id in the film_actor table. This is a one to many relationship between actor and film_actor table. `

2. What does the blue diamond next to the "last_update" column on the "inventory" table represent?

`the blue filled dimond next to the last_update column indicates not null simple attribute (menaing no key). `

3. How many foreign keys does the "payments" table have? How can you tell?

`the payment table has three foriegn keys. this is indicated by the blue lined diamond and listed under the indexes list with fk_[table_name]`
